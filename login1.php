...
<html>
<head>
<title>Login for User 1</title>
<link rel='stylesheet' href='login1.css'/>
</head>
<body>
<div id="d">
<h2>WELCOME TO LOGIN PANEL FOR USER 1</h2>
</div>
<br><br>
...
<form action='login1.php' method='post'>
	...
	<fieldset id='fs' align='center'>
		...
		<p>
		<label for='un'> User Name </label><input type='text' name='name' id='un' align='center'>
		</p>
		<p>
		<label for='p'> Password </label><input type='password' name='pass' id='p'>
		</p> 	
		<p>
		<label for='p'> Confirm Password </label><input type='password' name='pass1' id='p'>
		</p> 
...
<footer><a href="login2.php">LOGIN FOR USER 2</a></footer>
...
if(isset($_POST['submit']))
{
	...
	$ap = $_POST['pass'];
	$query = "select * from login1 where username='$an' AND password='$ap'"; 
    $run = mysql_query($query);
	if(@mysql_num_rows($run) == 1)
	{
		echo "<script>window.open('insert_data.php?logged=YOU ARE LOGGED IN ','_self')</script>";
	}	
	else
	{
...
if(isset($_POST['submit1']))
{
	...
	{
		$q="select * from login1 where username = '$un'";
		$run = mysql_query($q);
	...
	if($p == $cp)
	{
		$query = "insert into login1 (username,password) values ('$un','$p')";
		if(@mysql_query($query))
		{
			echo "<script>alert('New user signed up');
			window.open('login1.php?signup=New user has signed up....','_self');</script>";
		}
	}
	else
	{
